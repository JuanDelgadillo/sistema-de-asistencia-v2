...
if(isset($cedula))
{
    $administrativo = mysql_fetch_assoc(mysql_query("SELECT * FROM persona, administrativo WHERE persona.cedula = '$cedula' AND administrativo.cedula = persona.cedula "));
    $titulo = "<h1>Actualizar Administrativo /</h1><p>Formulario para actualizar un administrativo</p>";
    ...
    $administrativo['fecha_nac'] = explode("-",$administrativo['fecha_nac']);
    list($ano,$mes,$dia)=$administrativo['fecha_nac'];
    $grado_instruccion = $administrativo['grado_instruccion'];
    $turno = $administrativo['turno'];
    $especialidad = $administrativo['especialidad'];
    $area = $administrativo['area'];
}
else
{
    $titulo = "<h1>Registro Administrativo /</h1><p>Formulario para registrar un administrativo</p>";
                ...
                <div class="row">
                    <div class="col-sm-5 contact-form wow fadeInLeft">
                        <form role="form" name="registro_docente" action="<?=$action_form?>" method="post">
                            <input type="hidden" name="categoria" value="Administrativo">
